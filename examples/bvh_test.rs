...
pub fn main() {
    ...
    let min_point = Point::<f32, 3>::new(5.0, 5.0, 5.0);
    let max_point = Point::<f32, 3>::new(5.0, 5.0, 5.0);
    let query_aabb = Aabb::with_bounds(min_point, max_point);

    let overlapping_spheres = bvh.traverse_iterator(&query_aabb, &spheres);
    
    for sphere in overlapping_spheres {
        println!("{:?}", sphere);
    }

    //let hit_sphere_aabbs = bvh.traverse(&ray, &spheres);
    //dbg!(hit_sphere_aabbs);
    //dbg!(overlapping_spheres);
}