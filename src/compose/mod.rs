...
pub fn extrude(pts: &[Vec3], offset: Vec3) -> Result<Manifold, ManifoldError> {
    ...
    for p in pts.iter() {
        oft_ps.push(p + offset);
    }
    for i in idcs.iter() {
        oft_ts.push(Vec3u::new(i.z, i.y, i.x));
    }
    for i in idcs.iter() {
        oft_ts.push(Vec3u::new(i.x + n, i.y + n, i.z + n));
    }
    for i in 0..n {
        let j = (i + 1) % n;
