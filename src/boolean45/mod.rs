...
use crate::{Var, Manifold, Real, Vec3, Half, Tref, face_of};
...
fn size_output<T: Var>(
    ...
    let truncated = side_pq.iter().filter(|s| **s > 0).copied().collect::<Vec<_>>();
